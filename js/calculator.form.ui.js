...
function _validateField(input) {
  ...
  if (input.type === "number") {
    ...
    if (isNaN(value) && input.value.trim() !== "") {
      // Permite campo vazio se não for required
      isValid = false;
...
export function populateFormForEdit(tree) {
  if (!tree) return;
  console.log(
    "[DEBUG - calculator.form.ui] populateFormForEdit - tree object:",
    { id: tree.id, image_url: tree.image_url, hasPhoto: tree.hasPhoto },
  );
  const form = document.getElementById("risk-calculator-form");
  if (!form) return;
  ...
  if (tree.targetCategory) {
    ...
    const desktopRadio = document.querySelector(
      `input[name="target_category_desktop"][value="${tree.targetCategory}"]`,
    );
    ...
    const mobileRadio = document.querySelector(
      `input[name="target_category"][value="${tree.targetCategory}"]`,
    );
  ...
  if (tree.image_url) {
    ...
    db.getImageByUrl(tree.image_url)
          ...
          fetch(tree.image_url)
            ...
            .then((networkBlob) => {
              ...
              db.saveImageByUrl(tree.image_url, networkBlob) // Save to cache for next time
                .catch((cacheError) =>
                  console.error("Erro ao salvar imagem no cache:", cacheError),
                );
            })
            .catch((fetchError) => {
              console.error("Erro ao carregar foto da rede:", fetchError);
              showToast(
                `Falha ao carregar foto da rede para ID ${tree.id}.`,
                "error",
              );
            });
        }
      ...
      .catch((dbError) => {
        ...
        showToast(
          `Erro ao acessar cache local para foto de ID ${tree.id}.`,
          "error",
        );
      });
  } else {
...
function _setupFormListeners(form, isTouchDevice) {
  ...
  const resetBtnDesktop = document.getElementById(
    "reset-risk-form-btn-desktop",
  );
  const gpsStatus = document.getElementById("gps-status");
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", (e) => {
      ...
      state.setLastEvaluatorName(
        document.getElementById("risk-avaliador").value || "",
      );
      ...
      try {
        document.getElementById("risk-data").value = new Date()
          .toISOString()
          .split("T")[0];
        document.getElementById("risk-avaliador").value =
          state.lastEvaluatorName;
      } catch (err) {
        /* ignora */
      }
  ...
  // Listener de RESET para o botão duplicado do desktop
  if (resetBtnDesktop) {
...
export function showMobileQuestion(index) {
  const { questions, card, navPrev, navNext, counter, totalQuestions } =
    mobileChecklist;
...
export function setupMobileChecklist() {
  ...
  // Busca de elementos
  mobileChecklist.card = mobileChecklist.wrapper.querySelector(
    ".mobile-checklist-card",
  );
  mobileChecklist.navPrev =
    mobileChecklist.wrapper.querySelector("#checklist-prev");
  mobileChecklist.navNext =
    mobileChecklist.wrapper.querySelector("#checklist-next");
  mobileChecklist.counter =
    mobileChecklist.wrapper.querySelector(".checklist-counter");
  mobileChecklist.questions = document.querySelectorAll(
    "#risk-calculator-form .risk-table tbody tr",
  );

  // Validação
  if (
    mobileChecklist.questions.length === 0 ||
    !mobileChecklist.card ||
    !mobileChecklist.navPrev
  ) {
    return;
  }
  ...
  mobileChecklist.navPrev.parentNode.replaceChild(
    newNavPrev,
    mobileChecklist.navPrev,
  );
  ...
  mobileChecklist.navNext.parentNode.replaceChild(
    newNavNext,
    mobileChecklist.navNext,
  );
  ...
  mobileChecklist.card.addEventListener("change", (e) => {
    ...
    if (proxyCheckbox) {
      ...
      const realCheckbox =
        mobileChecklist.questions[targetIndex].cells[3].querySelector(
          ".risk-checkbox",
        );

...
function runTraqMatrices(failureProb, impactProb, targetCategory) {
    ...
    const eventLikelihood = (eventLikelihoodMatrix[failureProb] && impactIndex !== undefined) 
...
export async function handleAddTreeSubmit(event) {
    ...
    try {
        ...
        if (desktopTargetInput) currentRiskAssessment.targetCategory = desktopTargetInput.value;
        ...
        const mitigationMobile = document.getElementById('mitigation-action')?.value;
        ...
        if (state.currentTreePhoto && supabaseId) {
            try {
                ...
                const { data: uploadData, error: uploadError } = await ApiService.uploadImage(supabaseId, state.currentTreePhoto);
        ...
        if (RealtimeService.isSubscribed) {
            ...
            else if (mode === 'add') utils.showToast(`Registro ID: ${supabaseId} salvo.`, "success");
        ...
        form.reset();
        document.querySelectorAll('input[name^="target_category"]').forEach(r => r.checked = false);
...
export async function handleDeleteTree(id) {
    if (!confirm("Tem certeza que deseja excluir esta árvore permanentemente?")) return false;

    try {
        if (RealtimeService.isSubscribed) {
            const { error } = await ApiService.deleteTree(id);
            if (error) {
                utils.showToast("Erro ao excluir no servidor: " + error.message, "error");
                return false;
            }
        }

        const t = state.registeredTrees.find(tree => String(tree.id) === String(id));
        if (t && t.photoUrl) {
            await db.deleteImageByUrl(t.photoUrl);
        }

        const n = state.registeredTrees.filter(tree => String(tree.id) !== String(id));
...
export async function handleEditTree(id) {
    ...
    if (t.targetCategory) {
        const mobileTarget = document.querySelector(`input[name="target_category"][value="${t.targetCategory}"]`);
        const desktopTarget = document.querySelector(`input[name="target_category_desktop"][value="${t.targetCategory}"]`);
        if (mobileTarget) mobileTarget.checked = true;
        if (desktopTarget) desktopTarget.checked = true;
    ...
    if (t.mitigation) {
        ...
        if (mitMobile) mitMobile.value = t.mitigation;
        if (mitDesktop) mitDesktop.value = t.mitigation;
...
function getCSVData() {
  ...
  state.registeredTrees.forEach(t => {
    ...
    const r = [
      ...
      t.riskLevel, t.residualRisk, t.mitigation, 
...
export async function handleClearAll() {
    ...
    state.saveDataToStorage(); 
    try {
        if (db && typeof db.clearAllImages === 'function') await db.clearAllImages();
    } catch (e) {

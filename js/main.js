// js/main.js (v2.9.1 - Fix MapLibre Resize)
...
function handleMainNavigation(event, treeId = null) {
  ...
  // Gerenciar módulo de projetos
  if (targetId === "controle-projetos-view") {
    if (!projectsModule) {
      projectsModule = new ProjectsModule({
        container: document.getElementById('projects-module-container'),
...
async function openPlanningModule(treeId = null, planId = null) {
  let treesToProcess = state.registeredTrees;

  if (treeId) {
    treesToProcess = state.registeredTrees.filter((tree) => String(tree.id) === String(treeId));
  ...
  const trees = treesToProcess.map((tree) => ({
    ...
    failureProb: tree.failureProb || tree.failureProb || "-",
    targetType: tree.targetType || tree.impactProb || "-",
  ...
  // Se tiver planId, carregar o plano diretamente para edição
  if (planId) {
    console.log('[Main] 📝 Carregando plano para edição:', planId);

    // Primeiro fazer mount básico com config mínimo
    await PlanningModule.mount(
      "planning-module-root",
      {
        trees: [], // Será carregada pela função loadPlanById
    ...
    // Depois carregar o plano específico
    await PlanningModule.loadPlanById(planId);
    return;
  }

  // Lógica original para criar novo plano por árvore
  await PlanningModule.mount(
    "planning-module-root",
    {
      ...
      apiService: ApiService, // Passar ApiService para o módulo
      onSavePlan: (plan) => {
        // O salvamento agora é feito manualmente pelo usuário no módulo
...
// Exportar função globalmente para uso pelo módulo de projetos
window.openPlanningModule = openPlanningModule;
...
function setupActionButtons() {
  ...
  // Botão para ir ao Gestor de Planos
  const goToPlansBtn = document.getElementById("btn-go-to-plans");
  if (goToPlansBtn) {
    goToPlansBtn.addEventListener("click", () => {
      const plansNavBtn = document.querySelector(
        '.topico-btn[data-target="controle-projetos-view"]',
      );
      if (plansNavBtn) plansNavBtn.click();
    });
  }

  // Botão para retornar ao Gestor de Planos do editor de planos
  const returnToPlansBtn = document.getElementById("btn-return-to-plans");
  if (returnToPlansBtn) {
  ...
  try {
    ...
    window.addEventListener("resize", () => {
      ...
      if (mapContainer && mapContainer.offsetParent !== null) {
        ...
        // [FIX] Use resize() instead of invalidateSize() for MapLibre
...
// === SINCRONIZAÇÃO DE PLANOS NO LOGIN/LOGOUT ===
document.addEventListener('auth-user-logged-in', async () => {
  console.log('Usuário logado - sincronizando planos...');
  await syncPlansFromServer();
});

document.addEventListener('auth-user-logged-out', () => {
  console.log('Usuário deslogado - limpando planos locais...');
  clearLocalPlans();
});

async function syncPlansFromServer() {
  try {
    const result = await ApiService.getUserPlans();
    if (result.error) {
      console.error('Erro na sincronização de planos:', result.error);
      return;
    }

    // Armazenar planos sincronizados localmente
    localStorage.setItem('syncedPlans', JSON.stringify(result.data));
    console.log(`Sincronizados ${result.data.length} planos do servidor`);
...
function clearLocalPlans() {
  ...
  console.log('Planos locais removidos');
}

...
const getElements = () => {
  ...
  if (!container) {
    console.error("ChecklistMobileService: Container #checklist-flashcard-view não encontrado");
    console.error("Elementos disponíveis no DOM:", document.querySelectorAll('[id*="checklist"], [id*="flashcard"]'));
    return null;
  }
...
const showCurrentStepCard = () => {
  ...
  const currentStepIndex = checklistState.currentStep - 1; // currentStep é 1-based
  ...
  cards.forEach((card, index) => {
    ...
    // Força um reflow do navegador para garantir que a transição CSS seja aplicada
    void card.offsetHeight;

    if (index === currentStepIndex) {
      card.classList.add("is-active");
      // Força estilos inline para garantir visibilidade e centralização
      card.style.cssText = `
      ...
      switch (checklistState.currentStep) {
        ...
        case 4:
          renderConfirmation();
          // Adiciona listener ao botão de finalizar
          setTimeout(() => {
            const finishBtn = document.getElementById(
              "btn-finish-assessment-action",
            );
            if (finishBtn) {
  ...
  // Força visibilidade dos elementos de navegação
  if (els.stepper) {
    els.stepper.style.cssText = `
  ...
  if (els.navContainer) {
    els.navContainer.style.cssText = `
      display: flex !important;
      width: 100% !important;
      justify-content: space-between !important;
      gap: 15px !important;
...
const transferDataToForm = () => {
  ...
  if (checklistState.targetCategory) {
    const desktopRadios = form.querySelectorAll(
      'input[name="target_category_desktop"]',
    );
...
const openChecklist = () => {
  const els = getElements();
  if (!els) {
    console.error("ChecklistMobileService: Elementos não encontrados");
    return false;
  }

  console.log("ChecklistMobileService: Abrindo modal", els.container);
  ...
  // Força visibilidade do wrapper interno
  const wrapper = els.container.querySelector('.mobile-checklist-wrapper-fullscreen');
  if (wrapper) {
...
const initWithData = (treeData) => {
  ...
  const desktopMitigation = document.getElementById(
    "mitigation-action-desktop",
  );
  if (desktopMitigation)
    checklistState.mitigationAction = desktopMitigation.value;
...
export const ChecklistMobileService = {
  ...
  initWithData,
  init: () => {
    console.log("ChecklistMobileService: Inicializando serviço");
    // Verifica se os elementos existem no DOM
    const els = getElements();
    if (!els) {
      console.error("ChecklistMobileService: Falha na inicialização - elementos não encontrados");
      return false;
    }
    console.log("ChecklistMobileService: Serviço inicializado com sucesso");
    console.log("ChecklistMobileService: Container encontrado:", els.container);
    return true;
  },
  debug: () => {
    const container = document.getElementById("checklist-flashcard-view");
    if (container) {
      ...
      console.log("Estilo inline:", container.style.cssText);
      console.log("Visível:", container.offsetParent !== null);
      ...
      cards.forEach((card, index) => {
        console.log(`Card ${index + 1}:`, card ? card : "NÃO ENCONTRADO");
        if (card) {
          console.log(`  Classes: ${card.className}`);
          console.log(`  Visível: ${card.offsetParent !== null}`);
  ...
  forceShow: () => {
    ...
    if (container) {
      ...
      console.error("Container não encontrado para forçar exibição");
    }
  },

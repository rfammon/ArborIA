...
export async function handleAddTreeSubmit(event) {
  ...
  // --- REMOVIDA A FUNÇÃO sanitizeCoordinate: a validação e formatação de UTM é feita em utils.convertUtmToLatLon ---
  const treeData = {
    ...
    coordX: document.getElementById('risk-coord-x').value, // Passa o valor bruto do input
    coordY: document.getElementById('risk-coord-y').value, // Passa o valor bruto do input
    // Altera os fallbacks de UTM para null para que a validação em `convertUtmToLatLon` possa atuar corretamente
    utmZoneNum: (state.lastUtmZone && state.lastUtmZone.num) ? state.lastUtmZone.num : null,
    utmZoneLetter: (state.lastUtmZone && state.lastUtmZone.letter) ? state.lastUtmZone.letter : null,
...
export function handleEditTree(id) {
  ...
  if(state.setLastUtmZone) state.setLastUtmZone(t.utmZoneNum || null, t.utmZoneLetter || null); // Alterado para usar null
...
export function convertToLatLon(tree) { 
  // Esta função convertToLatLon no features.js parece ser uma duplicata ou versão mais antiga.
  // A principal lógica de conversão UTM->LatLon está em utils.js.
  // Será mantida por enquanto, mas revisada se causar inconsistências.
  if(tree.coordX === 'N/A' || tree.coordY === 'N/A') return null;
  // Assumir que tree.coordX e tree.coordY já são numéricos ou parseáveis para número.
  // No entanto, se eles vêm direto do input, podem ser strings.
  // O ideal seria que tree.coordX/Y já fossem numéricos ou que o parseamento fosse mais robusto aqui.

  const e = parseFloat(tree.coordX); 
  const n = parseFloat(tree.coordY);

  // Validação básica para evitar NaN ou valores claramente inválidos
  if (isNaN(e) || isNaN(n)) {
      console.warn("Coordenadas UTM (X, Y) não são números válidos para conversão no mapa.");
      return null;
  }

  const zn = tree.utmZoneNum || 23; // Fallback para zona, mas o ideal é que seja sempre válido
  const zl = tree.utmZoneLetter || 'S'; // Fallback para letra, ajustado para um valor mais comum no Brasil

  // Usar a função centralizada em utils.js para consistência
  const converted = utils.convertUtmToLatLon(e, n, zn, zl);
  if (converted) {
...
export function handleZoomToPoint(id) {
  ...
  const latLonCoords = convertToLatLon(t); // Esta chamada usará a versão atualizada acima
  if (!latLonCoords) {
...
export async function handleImportZip(event) {
  ...
  try {
    ...
    // Se ainda não encontrou, procura por qualquer CSV usando regex
    if (!csvFile) {
        const csvFilesFiltered = zip.filter(new RegExp(".*\\.csv$"));
        if (csvFilesFiltered.length > 0) {
            csvFile = csvFilesFiltered[0];
            // Assume formato novo se tiver 'traq' no nome do arquivo encontrado
            isTraqFormat = csvFile.name.toLowerCase().includes('traq');
        }
    }

    if (!csvFile) {
        throw new Error("Nenhum arquivo CSV válido (manifesto_dados_traq.csv, manifesto_dados.csv ou qualquer *.csv) encontrado no ZIP.");
    }
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (isTraqFormat) {
            ...
            tree = {
                ...
                utmZoneNum: parseInt(dataMap['ZonaN']) || null, // Alterado para null
                utmZoneLetter: dataMap['ZonaL'] || null, // Alterado para null
                dap: dataMap['DAP'], 
            ...
            tree = {
                ...
                utmZoneNum: parseInt(row[5]) || null, utmZoneLetter: row[6] || null, // Alterado para null
